...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    public function getRoles(): array
    {
        ...
        if (!empty($this->dbRoles)) {
            foreach ($this->dbRoles as $dbRole) {
                if (!is_string($dbRole) || $dbRole === '') {
                    continue;
                }
                $normalized = strtolower(trim($dbRole));
                if ($normalized === 'admin' || $normalized === 'role_admin') {
                    $roles[] = 'ROLE_ADMIN';
                    continue;
                }
                if (in_array($normalized, ['professor', 'instructor', 'teacher', 'role_professor', 'role_instructor', 'role_teacher'], true)) {
                    $roles[] = 'ROLE_PROFESSOR';
                    $roles[] = 'ROLE_INSTRUCTOR';
                    $roles[] = 'ROLE_TEACHER';
                    continue;
                }
                if (str_starts_with(strtoupper($dbRole), 'ROLE_')) {
                    $roles[] = strtoupper($dbRole);
                }
            }
        }

        if (is_string($this->role) && $this->role !== '') {
            $normalized = strtolower(trim($this->role));
            if ($normalized === 'admin' || $normalized === 'role_admin') {
                $roles[] = 'ROLE_ADMIN';
            } elseif (in_array($normalized, ['professor', 'instructor', 'teacher', 'role_professor', 'role_instructor', 'role_teacher'], true)) {
                $roles[] = 'ROLE_PROFESSOR';
                $roles[] = 'ROLE_INSTRUCTOR';
                $roles[] = 'ROLE_TEACHER';
            } elseif (str_starts_with(strtoupper($this->role), 'ROLE_')) {
    ...
    public function setDbRoles(array $roles): static
    {
        $this->dbRoles = $roles;
        return $this;
    }
